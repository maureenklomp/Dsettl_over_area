...
def create_restsettl_scatter(df, values, names, point_size, toggle_annotations, option_field):
    # Extract X, Y coordinates and values from the dataframe
    X_array = np.array(df['Easting'].values)  # latitudes
    Y_array = np.array(df['Northing'].values)  # longitudes
    Value_array = np.array(values)
    ...
    # Create color array based on value ranges
    colors = []
    if option_field == "restzetting (60 years-9 months)":    
        for val in Value_array:
            if val <= 0.125:
                ...
                colors.append('red')
    elif option_field == "eindzetting" or option_field == "zetting na 9 maanden":
        for val in Value_array:
            if val <= 0.30:
                colors.append('green')
            elif val <= 0.50:
    ...
    # Plot the original data points as scatter with color-coded points
    scatter = ax.scatter(X_array, Y_array, c=colors, s=point_size, edgecolor='black', linewidth=0.1, 
                        zorder=10)
    ...
    if option_field == "restzetting (60 years-9 months)":
        legend_elements = [
            Patch(facecolor='green', label='restzetting ≤ 125 mm'),
            Patch(facecolor='orange', label='125 mm < restzetting < 200 mm'),
            Patch(facecolor='red', label='restzetting > 200 mm')
        ]
    elif option_field == "eindzetting" or option_field == "zetting na 9 maanden":
        legend_elements = [
            Patch(facecolor='green', label='zetting ≤ 0.30 m'),
            Patch(facecolor='orange', label='0.30 m < zetting < 0.50 m'),
            Patch(facecolor='red', label='0.50 m < zetting < 0.80 m'),
            Patch(facecolor='purple', label='zetting > 0.80 m')
        ]

    ax.legend(handles=legend_elements, loc='upper left')

    if option_field == "restzetting (60 years-9 maanden)":
        ax.set_title('Restzetting (van 9 maanden tot 60 jaar')
    elif option_field == "eindzetting":
        ax.set_title('Eindzetting (na 60 jaar)')
    elif option_field == "zetting na 9 maanden":
        ax.set_title('Zetting na 9 maanden')
